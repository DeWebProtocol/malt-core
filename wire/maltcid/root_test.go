...
func TestSelfDescribingRoot(t *testing.T) {
	for _, id := range []maltcid.ProfileID{maltcid.KZG4096, maltcid.IPA256} {
		...
		c[0] = 42
		d := maltcid.RootDescriptor{Layout: maltcid.Prefix, InputRule: 1, Profile: id}
		...
		if root.Prefix().Codec != 0x300101 || maltcid.VersionIDOf(root) != 0 || !maltcid.IsMaltCid(root) {
		...
		if _, _, err := maltcid.ParseRoot(cid.NewCidV1(0x300101, hash)); err == nil {
			t.Fatal("nonminimal profile ID accepted")
		}
	}
	if _, err := maltcid.NewRoot(maltcid.RootDescriptor{Version: 1, Layout: maltcid.Prefix, Profile: maltcid.IPA256}, make([]byte, 32)); err == nil {
		t.Fatal("production V=1 enabled")
	}
	if _, err := maltcid.NewRoot(maltcid.RootDescriptor{Layout: maltcid.Positional, InputRule: 1, Profile: maltcid.IPA256}, make([]byte, 32)); err == nil {
		t.Fatal("Positional label rule accepted")
	}
}
