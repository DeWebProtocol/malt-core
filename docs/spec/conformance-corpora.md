...
The deterministic generator lives in `conformance/internal/generate`; its
command is `conformance/cmd/generate`. It is test tooling, not an SDK runtime
dependency. Native tests compare regenerated bytes with the checked-in corpus.

```bash
go run -p=6 ./conformance/cmd/generate -out conformance/authentication-v2.json
