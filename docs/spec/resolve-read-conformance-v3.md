# Resolve/Read Conformance Corpus V3

The v3 corpus is the language-neutral executable contract for local
verification of `malt.resolve/v0alpha1` and `malt.read/v0alpha1` values. The
canonical files live under `conformance/resolve-read/v3/` in this repository:
...
The corpus version is `malt.resolve-read.conformance/v3`, independent of the
enclosed operation profiles and Root `V=0`. Historical v1/v2 corpus bytes
remain in Git history and are not rewritten. Vector IDs, inputs, and expected
verdicts are frozen within a released corpus version. Error text and timings
are not conformance outputs.

## File And Envelope

`conformance/resolve-read/v3/vectors.json` has this shape:

```json
{
  "schema_version": "malt.resolve-read.conformance/v3",
  "vectors": [
    {
...
exact VC profile in the V0 multicommitment and validate its declared width;
...
V3 vectors keep these fields within the safe integer range so ordinary WASM
