// Package engine composes input rules, Prefix/Positional layouts and exact VC
// profiles. It owns no persistence, application path syntax or trusted roots.
package engine
...
func New(rules *input.Registry, profiles *Registry) *Engine {
	return &Engine{Rules: rules, Profiles: profiles}
}
...
func (e *Engine) config(d maltcid.RootDescriptor) (ProfileVerifier, maltcid.VCProfile, error) {
	if e == nil || e.Rules == nil {
		return nil, maltcid.VCProfile{}, errors.New("authentication engine is not configured")
	}
	if !e.Rules.Supports(input.ID(d.InputRule)) {
		return nil, maltcid.VCProfile{}, fmt.Errorf("unsupported input rule %d", d.InputRule)
	}
...
func (e *Engine) Interpret(state State) (View, error) {
	if _, _, err := e.config(state.Descriptor); err != nil {
		return View{}, err
	}
...
func (e *Engine) Commit(ctx context.Context, state View, out materializer.NodeUpdater) (cid.Cid, error) {
	...
	for i, entry := range state.Bindings {
		...
		if state.Descriptor.Layout == maltcid.Prefix && (k.Kind != input.Key || k.Index != 0) || state.Descriptor.Layout == maltcid.Positional && (k.Kind != input.Index || k.Key != ([32]byte{})) {
			return cid.Undef, errors.New("invalid authentication coordinate")
		}
		items[i] = binding{coordinate: k, target: entry.Target}
	}
	build := builder{ctx: ctx, descriptor: state.Descriptor, profile: p, scheme: prover, out: out}
...
func (b *builder) commit(cells []commitment.Cell) (maltcid.NodeRef, error) {
	...
	if err := b.out.PutNode(b.ctx, ref, cells); err != nil {
		return maltcid.NodeRef{}, err
	}
	return ref, nil
}
